...
```{r}
summary_by_cohort <- joined_release_2 %>%
  filter(!is.na(subject_id))  %>%
  group_by(cohort) %>%
...
We have `r length(joined_release_2$filename)` and with `r length(unique(joined_release_2$filename))` unique filenames, which means there are a few accidental duplicates.  
...
To sample videos for Whisper validation, we took a stratified sampling approach subsetted to monolingual, English speaking families, who comprised XX% of our dataset. For each subject, we then sampled videos that were 5 minutes or longer, and extracted a thirty-second clip from the middle of the video.
```{r}
...
write_csv(to_sample, file=here::here('to_sample_for_whisper_release2.csv'))
```
...
Then will need to sample these videos (from cluster) using python script in this folder.


```{r}
 # 
 # not_sampled <- video_duration %>%
 #   anti_join(to_sample) %>%
 #   filter(age_during_video<30) %>%
 #   filter(age_during_video>0)
 # 
 # vids_by_sub <- to_sample %>%
 #   group_by(sub_id) %>%
 #   summarize(num_vids = length(unique(full_paths)))
```
