...
class LSConverter:
    ...
    def __init__(self, config: str):
        ...
        labels_bbox = root.findall('.//RectangleLabels/Label')
        label_values_bbox = [label.get('value') for label in labels_bbox]
        labels_polygon = root.findall('.//PolygonLabels/Label')
        label_values_polygon = [label.get('value') for label in labels_polygon]
        label_values = label_values + label_values_bbox + label_values_polygon
    ...
    def convert_to_coco(self, input_json: str, output_json: str):
        ...
        for item_idx, item in enumerate(ann_list):
            ...
            image_name = item['data']['img'].replace('/data/local-files/?d=ccn2a/u/yinzi/babyview_20240507/babyview_main_pose_val_set/', '')
            ...
            for i, label in enumerate(item['annotations'][0]['result']):
                ...
                if 'rectanglelabels' == label['type'] or 'labels' == label[
                    ...
                    if x == y == 0:
                        current_kp[category_id] = [x, y, 0]
                        kp_num_change = 0
                    else:
                        current_kp[category_id] = [x, y, 2]
                        kp_num_change = 1
                    ...
                    if i == 0 or item['annotations'][0]['result'][
                        ...
                        annotations[-1]['keypoints'].update(current_kp)
                        kp_num += kp_num_change
        # resort the keypoints list in new order
        for ann in annotations:
            sorted_ann = dict(sorted(ann['keypoints'].items()))
            sorted_keypoints_list = [sorted_ann[key] for key in sorted_ann]
            ann['keypoints'] = np.array(sorted_keypoints_list).flatten().tolist()

        with io.open(output_json, mode='w', encoding='utf8') as fout:
            json.dump(
                {
                    'images': images,
                    'categories': [{"id": 0,"name": "bg"},{"id": 1,"name": "person"}],
